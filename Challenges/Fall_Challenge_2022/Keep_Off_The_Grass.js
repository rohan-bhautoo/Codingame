...
while (true) {
  ...
  const spawnTiles = myTiles.filter((tile) => tile.canSpawn);
  ...
  spawnTiles.sort((a, b) => a.spawnScore - b.spawnScore);

  const target = spawnTiles.shift();
  if (target && myMatter >= 10) {
    actions.push(`SPAWN ${1} ${target.x} ${target.y}`);
    myMatter -= 10;
  }
  while (turn >= endTurn && spawnTiles.length > 0 && myMatter >= 10) {
    const target = spawnTiles.shift();
    actions.push(`SPAWN ${1} ${target.x} ${target.y}`);
    myMatter -= 10;
  }
